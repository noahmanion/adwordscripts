...
function main() {
  ...
  var spreadsheet = SpreadsheetApp.create("Increase in CTR: " + accountName); 
  ...
  sheet.getRange("A2").setValue("Hero Pro: Increase in CTR");  
  ...
  while (adGroupsIterator.hasNext()) {
    ...
    // Week over week, the ad group is increasing - record that! 
  ...
  if (email) { 
    ...
    if(ctrRows.length>0){  
      ...
      subjectline.push(" Ad Group CTR's Are Increasing In " + accountName);  
      body.push("The CTR has increased over the last 3 weeks for ad groups in " + accountName + "." + " " + "Full report at: " + spreadsheet.getUrl());    
      MailApp.sendEmail(email, subjectline, body); 
    } 
  }

  else{ 
    body.push("No ad groups in " + accountName + " have consistently increasing CTR for the past three weeks. 
");  
    MailApp.sendEmail(email, subjectline, body); 
  } 
}
